...
moment.locale("es")
const DEFAULT_REPLY_CHIPS = ["Programar Entrevista‍", "Vacantes", "Turnos Laborales","¿Qué es Dialect?","Ubicación📍","Horarios Entrevista"];
...
function processV2Request(request, response) {
    ...
    const actionHandlers = {
                        ...
                        "fulfillmentMessages": [
                            {
                                ...
                                "text": {
                                    "text": [
                                        `¡Hola ${info.first_name}! Soy el robot de Dialect 👱‍‍ y estoy aquí para ayudarte a encontrar empleo.`
                                    ]
                                },
                            },
                            {
                            ...
                            "facebook": {
                                "attachment": {
                                    ...
                                    "payload": {
                                        ...
                                        "elements": [
                                            {
                                                "title": "Ubicación Dialect Talento",
        ...
        'activa.interview_generate': () => {
            console.log("Dialect.interview_generate");
            const request = require("request");
            return new Promise (resolve => {
                request(options, (error, response, body) => {
                    if (error) {
        ...
        'activainterview_generate.experience': () => {
            console.log("Dialect.interview_generate [Experience]");
            return new Promise (resolve => {
                const request = require("request");
                request(options, (error, response, body) => {
                    if (error) {
        ...
        'activainterview_generate.age': () => {
            console.log("Dialect.interview_generate [Age]");
            return new Promise (resolve => {
                const request = require("request");
                request(options, (error, response, body) => {
                    if (error) {
        ...
        'activainterview_generate.gender': () => {
            console.log("Dialect.activainterview_generate [GENDER]");
            return new Promise (resolve => {
                const request = require("request");
                request(options, (error, response, body) => {
                    if (error) {
        ...
        'activainterview_generate.turno': () => {
            console.log("Dialect.activainterview_generate [Turno]");
            return new Promise (resolve => {
                const request = require("request");
                request(options, (error, response, body) => {
                    if (error) {
        ...
        'activainterview_generate.end': () => {
            console.log("Dialect.activainterview_generate [END]");
            return new Promise (resolve => {
                const request = require("request");
                request(options, (error, response, body) => {
                    if (error) {
